...
! !modtg
! subroutine CCint2_matS(ABtype,add_t,matS,nbas)
!   !This subroutine is now defunct. Look into ggint.f90
!   implicit none 
!   character(1),intent(in) :: ABtype
!   integer,intent(in) :: add_t,nbas
!   real(prec) :: matS(:,:)
!   ! integer :: i,j,ii,jj,kk,ll

!   ! print*, "after we just czek"
!   ! print*, "intf122,1234",intf122chem(1,2,3,4)
!   ! print*, "intf122,2143",intf122chem(2,1,4,3)
!   ! print*, "intf122,2134",intf122chem(2,1,3,4)
!   ! call create_matS_file(nbas)
!   ! stop
!   ! do i = 1, nbas**2
!   !    ii=modulo(i-1,nbas)
!   !    kk=(i-1)/nbas
...
!   !       jj=modulo(j-1,nbas)
!   !       ll=(j-1)/nbas
!   !       if (add_t.eq.0) matS(i,j)=0._prec
!   !       if (ABtype.eq."A") then 
!   !          matS(i,j)=intf122chem(ii,kk,jj,ll)
!   !       else if (ABtype.eq."B") then
!   !          matS(i,j)=intf122chem(ii,kk,ll,jj)
!   !       else
