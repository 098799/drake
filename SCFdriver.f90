...
! do i = 1, nbas**2
!    do j = 1, nbas**2
!       if (abs(twoel(i,j)-14.33).LT.1E-7) then
!          print*, modulo(i-1,nbas),(i-1-modulo(i-1,nbas))/nbas,modulo(j-1,nbas),(j-1-modulo(j-1,nbas))/nbas,twoel(i,j)
!       end if
!    end do
! end do
! stop
! do i = 1, nbas**2
!    do j = 1, nbas**2
!       if (twoel(i,j)-twoell(i,j).GT.0.01_prec) print*, i,j,twoel(i,j),twoell(i,j)
!    end do
! end do
